...
- 一覧は魚ごとのカードで表示する。広い画面では3列、中間幅では2列、スマホでは1列に切り替える。
- 上部に発見済み魚種数と完成率のバーを表示し、分類フィルタにも発見数／総数を添える。
- 魚名検索と分類・海域・季節の絞り込みを維持する。絞り込み欄は折りたたみ式にする。
...
- 釣り・釣り小屋は地図カード内に表示し、「地図に戻る」で閉じる。図鑑・餌購入・魚売却・竿報酬は既存の画面を活用する。
- 竿・残り回数、水面と状態説明、操作欄を共通の配置とする。開始・投げる・引く・結果から次へ進むボタンは操作欄の右側に固定する。スマホでも同じ2列配置で収める。
- 水面は控えめな波と浮きで表現し、待機中とアタリ中で浮きの動きを変える。動きを減らす設定ではアニメーションを停止する。
- 釣果確定時は魚名・サイズを大きく表示し、初釣果・最大記録更新をバッジで示す。失敗時には魚名・サイズを公開しない。
- 釣果と餌の一覧は折りたたみ式とし、街・村の管理画面や最終釣果の確認時には開く。
- 別の行動、場所・モードの変更で釣り画面を閉じる。待機中・アタリ中に閉じた場合の破棄と、消費済みの回数を戻さない仕様は維持する。
