...
async function main() {
  ...
  delete state.nationalPower;
  state.year = 1005; state.season = 2; state.day = 19;
  assert.equal(saveGameToStorage(), true);
  assert.equal(loadGameFromStorage(), true);
  assert.equal(state.nationalPower.values.north, 600000);
  assert.equal(state.nationalPower.lastProcessedAbs, 120679);
  state.nationalPower.values.north = 0;
  state.nationalPower.values.citadel = 723456;
  const powerBefore = JSON.stringify(state.nationalPower);
  assert.equal(saveGameToStorage(), true);
  resetState();
  assert.equal(loadGameFromStorage(), true);
