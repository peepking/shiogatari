...
export function renderFaithDetails() {
  ...
  body.innerHTML = `<div class="faith-current"><span>現在の信仰</span><strong>${Math.max(0, Number(state.faith) || 0).toLocaleString()}</strong></div><div class="faith-benefit-list"><section class="faith-benefit-card"><h3>各地の支え</h3><p class="faith-caption">潮盟全体の今季</p><dl class="faith-metrics"><div><dt>恩恵対象</dt><dd>${targets.length}<small>拠点</small></dd></div><div><dt>信仰 / 季節</dt><dd>${seasonalFaith}</dd></div><div><dt>神託報酬</dt><dd>＋${state.tideAlliance?.bonus || 0}<small>%</small></dd></div></dl><p class="tiny">季節初日に恩恵対象と効果を固定します。信仰の端数は繰り越されます（現在 ${(state.tideAlliance?.remainder || 0) / 10}）。</p></section>${effects.afterglow ? `<div class="faith-benefit-card"><h3>祈りの余潮</h3><p>交易・維持費の恩恵が${FAITH_CONFIG.afterglow}倍</p><p class="tiny">${deadline}</p></div>` : ''}${rows.map(([name, description]) => `<section class="faith-benefit-card"><h3>${name}</h3><p>${description}</p></section>`).join('') || '<p>現在、適用中の恩恵はありません。</p>'}</div><p class="faith-footnote tiny">潮盟は、潮語りの言葉に耳を傾ける「潮の縁者」の共同体です。恩恵は現在の信仰に応じて変わります。</p>`;
}
