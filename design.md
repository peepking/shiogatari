...
- 追加仕様全体と未実装範囲は `expansion-design.md` を参照。
...
- `outfitting.js`: 装備中の効果集計、上限の割合増加、消費合計の割合軽減、補助兵効果の10人上限。現段階では純粋計算のみでゲーム本体には未適用。
...
- 完成地点は割当時の現在地から距離10～20で内部固定し、完成まで描画・座標表示しない。噂は拠点の季節初回入場で10%、距離10～15。同一海図に1枚まで。探索・海図の予約座標は後から生成する討伐地点の候補からも除外する。
